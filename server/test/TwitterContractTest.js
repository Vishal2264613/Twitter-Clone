...
describe("Twitter Contract", function () {
  ...
  this.beforeEach(async function () {
    ...
    for (let i = 0; i < NUM_TOTAL_MY_TWEETS; i++) {
      let tweet = {
        tweetText: "Random text with id-" + (NUM_TOTAL_MY_TWEETS + i),
        username: owner,
        isDeleted: false,
      };
  ...
  describe("Add Tweet", function () {
    it("should emit the AddTweet event", async function () {
      let tweet = {
        tweetText: "New Tweet",
        isDeleted: false,
      };
  ...
  describe("delete Tweet", function () {
    it("should emit the delete tweet event", async function () {
      const TWEET_ID = 0;
